...
func (p Proxy) ServeHTTP(w http.ResponseWriter, req *http.Request) {
	...
	if proxyRequestURI, _ := url.ParseRequestURI(req.Header.Get("X-Forwarded-Uri")); proxyRequestURI != nil {
		...
		prefix = filepath.Join(p.ProxyPrefix, strings.TrimSuffix(req.URL.Path, requestpath))
	}
...
var _ ErrorResponder = DefaultErrorResponder{}

type DefaultErrorResponder struct{}

func (DefaultErrorResponder) Error(w http.ResponseWriter, req *http.Request, err error) {
	dnse := &net.DNSError{}
