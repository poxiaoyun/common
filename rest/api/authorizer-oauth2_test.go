...
func TestAuthorizationFilterUsesOAuth2ScopeAuthorizerInChain(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			authentication: api.AuthenticationInfo{
				...
				Access:  &api.AccessConstraints{Scopes: []string{"orders.read"}},
			},
			action:            "get",
			fallbackDecision:  api.DecisionDeny,
		...
		},
		{
			...
			authentication: api.AuthenticationInfo{
				Subject: api.Subject{ID: "client"},
				Access:  &api.AccessConstraints{Scopes: []string{"orders.write"}},
			},
			action:            "get",
			fallbackDecision:  api.DecisionAllow,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			fallbackCalls := 0
			authorizer := api.AuthorizerChain{
				api.OAuth2ScopeAuthorizer{MatchScope: func(scope string, attributes api.Attributes) bool {
					return scope == "orders.read" && attributes.Action == "get"
				}},
				api.AuthorizerFunc(func(context.Context, api.AuthenticationInfo, api.Attributes) (api.Decision, string, error) {
					fallbackCalls++
...
func TestAuthorizationFilterDoesNotReportBusinessDenialAsInsufficientScope(t *testing.T) {
	response := httptest.NewRecorder()
	request := httptest.NewRequest(http.MethodGet, "/orders", nil)
	request = request.WithContext(api.WithAuthentication(request.Context(), api.AuthenticationInfo{
		Subject: api.Subject{ID: "client"},
		Access:  &api.AccessConstraints{Scopes: []string{"orders.read"}},
	}))
	request = request.WithContext(api.WithAttributes(request.Context(), &api.Attributes{Action: "get"}))

	filter := api.NewAuthorizationFilter(api.AuthorizerFunc(func(context.Context, api.AuthenticationInfo, api.Attributes) (api.Decision, string, error) {
		return api.DecisionDeny, "denied by resource policy", nil
	}))
...
func TestDefaultOAuth2ScopeMatcher(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			actual := api.DefaultOAuth2ScopeMatcher(test.input.scope, test.input.attributes)
...
func TestNewOAuth2ScopeMatcher(t *testing.T) {
	matchResource := func(grantedResource string, attributes api.Attributes) bool {
		return grantedResource == "catalog" && attributes.Resources[len(attributes.Resources)-1].Resource == "flavors"
	}
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			if actual := match(test.scope, test.attributes); actual != test.expected {
