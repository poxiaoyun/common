...
固定 Client Credentials 通过 `ClientOptions.Authentication` 配置；`NewClientCredentialsRoundTripper` 为下游请求获取并缓存短期 Access Token。该 transport 会克隆请求并覆盖出站 `Authorization`，因此调用方或入站请求的凭据不会被透传给目标 Resource Server。
